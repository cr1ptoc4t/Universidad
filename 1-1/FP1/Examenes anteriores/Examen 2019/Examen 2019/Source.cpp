...
bool capacidad(int i, tListaMes lm, int com);
...
int buscarMesa(tListaMes lm, int com, char zona) {
	int i = 0;
	while (!(capacidad(i, lm, com) && lm.lm[i].reservada == false && zona == lm.lm[i].ubi) && i<lm.cont) { i++; }
...
bool capacidad(int i, tListaMes lm, int com) {
	bool b = false;
	if (lm.lm[i].cap == com || lm.lm[i].cap + 1 == com) b = true;
