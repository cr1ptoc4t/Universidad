/**
 ...
 Modificado en 2022 por José Luis Sierra para poder declarar iteradores no inicializados.
*/
...
public:
	...
	Lista extrae_e_inserta(int pos_ini, int n_elems_a_insertar, Lista b) {
		if (pos_ini < _numElems && n_elems_a_extraer>0 && !n_elems_a_insertar.esVacia()) {
			Lista extracto;
			Nodo* inic = _prim;
			while (pos_inicio > 0) {
				inic = inic->sig;
			...
			Nodo* fin = inic;
			while (n_elems_a_extraer > 0 && fin != nullptr) {
				fin = fin->_sig;
				n.elems_a_extraer--;
				extracto._numElems++;
			}

			if (inic->_ant != nullptr) {
				inic->_ant = elems_a_insertar->prim;
			}
			else {
				_prim = elems_a_insertar->_prim;
			}
			elems_a_insertar._prim->_ant = inic->_ant;

			if (fin != nullptr)
				elems_a_insertar._ult->_sig = fin->_sig;
			else {
				_ult = elems_a_insertar._ult;
				elems_a_insertar._ult->_sig = nullptr;
			}

			elems_a_insertar.prim->_ant = inic->_ant;

			if (fin != nullptr) {
				elems_a_insertar._prim->sig = fin->_sig;

				if (fin-> _sig != nullptr) {
					fin->_sig->_ant = elems_a_insertar._ult;
				}
			}
			else
				ult = elems_a_insertar._ult;

			elems_a_insertar._ult = nullptr;
			elems_a_insertar._prim = elems_a_insertar._ult;
